...
#define DMA_RAM_D1 __attribute__((section(".bss.RAM_D1")))
#elif defined(__GNUC__)
#define DMA_RAM_D1 __attribute__((section(".RAM_D1")))
#else
#define DMA_RAM_D1
#endif
...
/* USER CODE BEGIN PV */
volatile UINT azure_rtos_init_step;
volatile UINT azure_rtos_init_status;
...
VOID tx_application_define(VOID *first_unused_memory)
{
  ...
  else
  {
    ...
    azure_rtos_init_step = 2;
    status = App_ThreadX_Init(memory_ptr);
    azure_rtos_init_status = status;
  ...
  /* FileX is temporarily disabled while validating ThreadX/blink startup. */
  /*
  ...
  */
  ...
  /* NetXDuo is initialized later from a ThreadX thread so link problems cannot
     block kernel startup or the blink thread. */
